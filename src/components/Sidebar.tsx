...
import { ingestDocumentAction, ingestPdfAction } from "@/app/actions";
...
export function Sidebar({ documents, onIngestSuccess }: SidebarProps) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      if (file.type === "application/pdf" || file.name.endsWith(".pdf")) {
        // PDF path: Send binary to server
        const formData = new FormData();
        formData.append("file", file);
        await ingestPdfAction(formData);
      } else {
        // Text path: Read locally then send content
        ...
        return; // handle async end inside onload
      }
      onIngestSuccess();
    } catch (err) {
  ...
  return (
    <aside className="w-80 border-r border-slate-200 bg-white flex flex-col shrink-0 shadow-sm font-sans">
      <input 
        type="file" 
        ref={fileInputRef} 
        className="hidden" 
        onChange={handleFileUpload}
        accept=".txt,.md,.js,.ts,.json,.pdf"
      />
            ...
            >
              {isIngesting ? (
                <span className="flex items-center gap-2">
                   <div className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Ingesting...
                </span>
              ) : (
                <><PlusIcon className="w-4 h-4" /> Ingest Doc</>
              )}
            </button>
          </form>
        </div>
